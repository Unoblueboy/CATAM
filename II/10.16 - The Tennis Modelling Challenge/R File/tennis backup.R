...
d["Federer R."] = 1
d["Murray A."] = -1
# new_dat = as.data.frame(t(d))
# pred <- predict(TennisGLM1, newdata = new_dat, type="link", se.fit = TRUE)
# pred <- predict(TennisGLM1, newx = matrix(d, nrow=1), s = 0, type="link")
pred <- t(d) %*% coefs
...
# CI_prob_Bounds <- invlogit(c(pred$fit - critval*pred$se.fit, pred$fit + critval*pred$se.fit))
...
# Note that for this model as we have set all Y to 1 the log likelihood is
# equal to log of the inverse logit function
LL3 = 0
for (i in 1:Training_length) {
  row_X2 = X2[i, ]
  X2b = row_X2 %*% coefs2
...
# result <- anova(TennisGLM1, TennisGLM2, test="LR")
...
# print(paste("P-value for the likelihood ratio test is:",result$`Pr(>Chi)`))
print(paste("P-value for the likelihood ratio test is:",p_val))
# Extremely small P value (< 0.01), so reject null hypothesis, there is sufficient evidence to suggest
# That the second model is better
...
# QUESTION 9
# If we want to test this model using log-loss for cross-validation, then we need a training set and 
# Test Set that includes all the years, we will do this by getting all the matches for each year
# and putting 9/10 into the training set and the rest into the test set
# We need to be careful with test set as  it is entirely possible that a player isn't in the 
# Training set for a year but in the test set
# We can fix this by having the training set such that if a player plays during a certain year, it must
# have at least one entry in the training set, this ensures that all possible 

Training_Data_2 <- data.frame(matrix(ncol = dim(Tennis)[2], nrow = 0))
Test_Data_2 <- data.frame(matrix(ncol = dim(Tennis)[2], nrow = 0))
colnames(Training_Data_2) <- colnames(Tennis)
colnames(Test_Data_2) <- colnames(Tennis)

for (year in 2000:2016) {
  start_date = as.Date(paste0(year, "/01/01"))
  end_date = as.Date(paste0(year, "/12/31"))
  year_data <- Tennis[(Tennis$Date >= start_date) & (Tennis$Date <= end_date)  & (!is.na(Tennis$Date)), ]
  rownames(year_data) <- 1:dim(year_data)[1]
  
  year_names = union(unique(year_data$Loser), unique(year_data$Winner))
  temp_year_Data <- data.frame(matrix(ncol = dim(Tennis)[2], nrow = 0))
  colnames(temp_year_Data) <- colnames(Tennis)
  while ((length(year_names) != 0) && (dim(year_data)[1] != 0) ) {
    name = year_names[1] # sample
    temp_data = year_data[(year_data$Winner == name) | (year_data$Loser == name), ]
    temp_datum = temp_data[1, ] # sample
    year_names <- year_names[year_names != temp_datum$Winner]
    year_names <- year_names[year_names != temp_datum$Loser]
    year_data <- year_data[apply(year_data, 1, function(x) !all(x['Winner']==temp_datum$Winner, 
                                                                x['Loser']==temp_datum$Loser, 
                                                                x['Date']==temp_datum$Date)), ]
    temp_year_Data <- rbind(temp_year_Data, temp_datum)
  }
  
  num_entries <- dim(year_data)[1]
  year_test_data_indices = 1:floor(num_entries/10) # sample
  year_test_data = year_data[year_test_data_indices, ]
  year_training_data = rbind(year_data[-year_test_data_indices, ], temp_year_Data)
  Test_Data_2 <- rbind(Test_Data_2, year_test_data)
  Training_Data_2 <- rbind(Training_Data_2, year_training_data)
  # print(paste("Year",year,"done."))
}

# Create the Design Matrix

Training_2_length = dim(Training_Data_2)[1]
Test_2_length = dim(Test_Data_2)[1]

Training_2_winners <- as.vector(Training_Data_2$Winner)
Training_2_losers <- as.vector(Training_Data_2$Loser)
Test_2_winners <- as.vector(Test_Data_2$Winner)
Test_2_losers <- as.vector(Test_Data_2$Loser)
Training_2_Y <- rep(c(1,0), length.out = Training_2_length)
Test_2_Y <- rep(c(1,0), length.out = Test_2_length)

tennis_years = 2000:2016
column_names = rep("",(length(tennis_names)-1)*(length(tennis_years)))
X3 <- matrix(data=0, nrow=Training_2_length, ncol = length(column_names))
Test_X3 <- matrix(data=0, nrow=Test_2_length, ncol = length(column_names))
for (i in 2:length(tennis_names)) {
  column_names[length(tennis_years)*(i-2)+1] <- tennis_names[i]
  for (j in 2:length(tennis_years))
    column_names[length(tennis_years)*(i-2)+(j)] <- paste(tennis_names[i], tennis_years[j])
}
colnames(X3) <- column_names
colnames(Test_X3) <- column_names

Training_2_Date <- Training_Data_2$Date
Test_2_Date <- Test_Data_2$Date

for (i in 1:Training_2_length) {
  winner = Training_2_winners[i]
  loser = Training_2_losers[i]
  year = as.numeric(format(Training_2_Date[i],'%Y'))
  y_factor = 2*Training_2_Y[i]-1
  if (winner != tennis_names[1]) {
    X3[i, winner] <- 1*y_factor
    if (year != tennis_years[1]) {
      X3[i, paste(winner, year)] <- 1*y_factor
    }
  }
  if (loser != tennis_names[1]) {
    X3[i, loser] <- -1*y_factor
    if (year != tennis_years[1]) {
      X3[i, paste(loser, year)] <- -1*y_factor
    }
  }
}
# Delete empty columns
...
# OK, so we are going to weight this in a weird way
# We are going to have all the base player things be unweighted
# But then we want to acknowledge that during a single year there won't be 
# much change, but over several there is likely to be
# so going to have a (linear or exponential) decrease in the weight
# as the year increases for each player
w2 <- rep(c(0,exp(seq(0, -10, length.out = length(tennis_years)-1))), length(tennis_names)-1)
names(w2) <- column_names
w2 <- w2[names(w2) %in% colnames(X3)]

TennisLassoGLM2 <- glmnet(X3, Training_2_Y, family="binomial", lambda = lam_seq, 
...
coefs4 <- coefs4[2:length(coefs4)]

for (i in 1:Test_2_length) {
  winner = Test_2_winners[i]
  loser = Test_2_losers[i]
  year = as.numeric(format(Test_2_Date[i],'%Y'))
  y_factor = 2*Test_2_Y[i]-1
  if (winner != tennis_names[1]) {
    Test_X3[i, winner] <- 1*y_factor
    if (year != tennis_years[1]) {
      Test_X3[i, paste(winner, year)] <- 1*y_factor
    }
  }
  if (loser != tennis_names[1]) {
    Test_X3[i, loser] <- -1*y_factor
    if (year != tennis_years[1]) {
      Test_X3[i, paste(loser, year)] <- -1*y_factor
    }
  }
}
...
for (i in 1:Training_2_length) {
  row_X3 = X3[i, ]
  X3b = row_X3 %*% coefs4
  term = Training_2_Y[i]*log(invlogit(X3b)) - (1-Training_2_Y[i])*K(X3b)
  LL7 = LL7 + term
}
LL7 = LL7*-1/Training_2_length
...
for (i in 1:Test_2_length) {
  row_X3 = X3[i, ]
  X3b = row_X3 %*% coefs4
  term = Test_2_Y[i]*log(invlogit(X3b)) - (1-Test_2_Y[i])*K(X3b)
  LL8 = LL8 + term
}
LL8 = LL8*-1/Test_2_length
print(paste("TennisLassoGLM2 Log Loss for Test Set:", LL8))

Fed_Nad_Mat = matrix(0, nrow = 14, ncol=dim(X3)[2])
colnames(Fed_Nad_Mat) <- colnames(X3)
for (year in 2003:2016) {
  Fed_Nad_Mat[year - 2002, "Federer R."] = 1
  Fed_Nad_Mat[year - 2002, paste("Federer R.", year)] = 1
  Fed_Nad_Mat[year - 2002, "Nadal R."] = -1
  Fed_Nad_Mat[year - 2002, paste("Nadal R.", year)] = -1
}
plot(2003:2016, invlogit(Fed_Nad_Mat %*% coefs4),
     xlab="Year", ylab="Probability Federer beats Nadal")
lines(2003:2016, invlogit(Fed_Nad_Mat %*% coefs4))

# Note that there is no way to directly test (that we have learnt) models where the parameter spaces aren't
# subsets of one another, so we will need to use some sort of proxy, in this case we can use the log-loss on
# Training set 2
...
KB7 = LL7 + length(coefs4)/Training_2_length
KB8 = LL8 + length(coefs4)/Test_2_length
...
# The fact that TennisLassoGLM2 has lowest LL on training set but highest LL on test set
# suggests that the model overfits to the training set
...
portfolio <- function(v, coeffs) {
  ...
  # Not Solve.QP isn't great as it may still have -ve amounts in the solution
...
legend("right", legend = c("TennisGLM2", "TennisLassoGLM1"), col=c("black", "red"), lty=c(1, 1), inset=0.05)

# The fact that we lose money as we increase the impact of risk is due to the fact that
# increasing the risk promotes distributing the money more equally so are less likely to
# gain lots of money off of a single bet
# The fact that as v goes to 0 the profit goes to -1000 indicates that all the money is going to
# a match that has a predicted high likelihood of being won but is actually lost
# the max at v=100 indicates this is a good value to balance out the risk and reward
